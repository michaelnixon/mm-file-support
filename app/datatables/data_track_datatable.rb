  ...
  def data
    records.map do |record|
      [
        ...
        link_to(truncate(record.name, length: 40), data_track_path(record),class:"underlined"), record.take.try(:name), record.sensor_types.map{|type| link_to(type.name, sensor_type_path(type),class:"underlined") }.join(", "), record.movers.map{|mover| link_to(mover.name, mover_path(mover),class:"underlined") }.join(", "),
